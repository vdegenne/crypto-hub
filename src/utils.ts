...
export function makeChatGPTRequest(
	requestWithPlaceholders: string,
	currency: string | CMC.MiniCurrency | undefined | null,
) {
	if (!currency) {
		return "Sorry I didn't mean to do this request."
	}

	return requestWithPlaceholders
		.replace(
			/\%name/g,
			(currency as CMC.MiniCurrency)?.name ?? (currency as string),
		)
		.replace(
			/\%symbol/g,
			(currency as CMC.MiniCurrency)?.name ?? (currency as string),
...
export function setFaviconFromImage(img: HTMLImageElement): void {
	...
	// Find a link rel="icon" without sizes (our dynamic favicon)
	let link = Array.from(
		document.querySelectorAll<HTMLLinkElement>("link[rel='icon']"),
	).find((l) => !l.hasAttribute('sizes'))

	if (!link) {
		// Create a new dynamic favicon link if none exists
